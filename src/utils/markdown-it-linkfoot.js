...
function renderFootnoteCaption(tokens, idx) {
  ...
  // 添加标题标签的包裹
  return `<span class="footnote-caption">[${n}]</span>`;
}
...
function renderFootnoteBlockOpen(tokens, idx, options) {
  // 旧代码生成 <h3 class="footnotes-sep"></h3>
  // 新代码生成带标题的分隔区块
  return `<section class="footnotes-sep">
...
function renderFootnoteOpen(tokens, idx, options, env, slf) {
  ...
  // 旧代码：<span id="fn' + id + '" class="footnote-item"><span class="footnote-num">[...]
  // 新代码保持结构但使用更清晰的格式化
  return `<span id="fn${id}" class="footnote-item">
